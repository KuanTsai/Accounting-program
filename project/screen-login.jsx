const { useState: useStateLogin } = React;

function LoginScreen() {
  const [loading, setLoading] = useStateLogin(false);
  const [error, setError] = useStateLogin('');

  const handleLogin = async () => {
    setLoading(true);
    setError('');
    try {
      await window.auth.signInWithPopup(window.googleProvider);
    } catch (e) {
      setError('登入失敗，請再試一次');
      setLoading(false);
    }
  };

  return (
    <div className="paper-bg" style={{
      position: 'fixed', inset: 0,
      display: 'flex', flexDirection: 'column',
      alignItems: 'center', justifyContent: 'center',
      padding: '40px 32px',
    }}>
      {/* logo area */}
      <div style={{
        width: 100, height: 100, borderRadius: 32,
        background: 'linear-gradient(135deg, var(--accent) 0%, var(--secondary) 100%)',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        boxShadow: '0 12px 32px rgba(255,143,171,0.4)',
        fontSize: 52, marginBottom: 28,
      }}>🦊</div>

      <div style={{ textAlign: 'center', marginBottom: 48 }}>
        <div className="hand" style={{ fontSize: 30, color: 'var(--ink)', lineHeight: 1.3 }}>
          小桃の記帳日記
        </div>
        <div style={{ fontSize: 13, color: 'var(--ink-soft)', marginTop: 8 }}>
          Today is a lovely day ✿
        </div>
      </div>

      <div className="tap" onClick={loading ? null : handleLogin} style={{
        display: 'flex', alignItems: 'center', gap: 12,
        padding: '14px 28px', borderRadius: 99,
        background: '#fff', cursor: loading ? 'default' : 'pointer',
        boxShadow: '0 4px 16px rgba(0,0,0,0.10)',
        fontSize: 15, fontWeight: 600, color: '#444',
        opacity: loading ? 0.7 : 1,
        transition: 'opacity 0.15s',
      }}>
        <svg width="20" height="20" viewBox="0 0 24 24">
          <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
          <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
          <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
          <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
        </svg>
        {loading ? '登入中…' : '用 Google 帳號登入'}
      </div>

      {error && (
        <div style={{ marginTop: 16, fontSize: 13, color: 'var(--accent)' }}>{error}</div>
      )}

      <div style={{ position: 'absolute', bottom: 32, fontSize: 12, color: 'var(--ink-faint)' }}>
        資料只屬於你 · 安全加密儲存
      </div>
    </div>
  );
}

Object.assign(window, { LoginScreen });
